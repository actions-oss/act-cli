...
//nolint:unparam,errcheck,deadcode
package container
...
func parse(flags *pflag.FlagSet, copts *containerOptions, serverOS string) (*containerConfig, error) {
	...
	for bind := range copts.volumes.GetMap() {
		parsed, err := parseVolumeSpec(bind)
		if err != nil {
			return nil, err
		}
...
func convertPortSet(ports nat.PortSet) (network.PortSet, error) {
	converted := make(network.PortSet, len(ports))
	for port := range ports {
		p, err := network.ParsePort(port.Port() + "/" + port.Proto())
		if err != nil {
			return nil, err
		}
		converted[p] = struct{}{}
...
func convertPortMap(bindings nat.PortMap) (network.PortMap, error) {
	converted := make(network.PortMap, len(bindings))
	for port, portBindings := range bindings {
		p, err := network.ParsePort(port.Port() + "/" + port.Proto())
		if err != nil {
			return nil, err
		}
		convertedBindings := make([]network.PortBinding, 0, len(portBindings))
		for _, binding := range portBindings {
			...
			convertedBindings = append(convertedBindings, network.PortBinding{
				HostIP:   hostIP,
				HostPort: binding.HostPort,
			})
		}
		converted[p] = convertedBindings
	}
	return converted, nil
}
